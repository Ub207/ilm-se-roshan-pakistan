...
import { SparkIcon, SUBJECT_ICONS } from "@/components/icons";
import { MAX_TOPIC_LENGTH, MCQ_COUNT, isSubject } from "@/lib/tutor";
...
export default async function TutorPage({ searchParams }: PageProps<"/tutor">) {
  ...
  const SubjectIcon = subject ? SUBJECT_ICONS[subject] : null;
    ...
    <main className="mx-auto w-full max-w-3xl px-4 py-8 sm:px-6 sm:py-12">
      <div className="mb-6 sm:mb-8">
        ...
        <h1 className="mt-2 text-3xl font-bold tracking-tight sm:text-4xl">
          {subject
            ? `${subject} — koi bhi topic poochein`
            : "Koi bhi topic poochein"}
        </h1>
        <p className="mt-2.5 max-w-2xl leading-relaxed text-muted">
          Aap topic likhein, AI aap ko simple English aur Roman Urdu mein
          samjhayega — phir {MCQ_COUNT} practice questions aur agla lesson bhi
          dega.
        </p>
        {subject && SubjectIcon && (
