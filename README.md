...
| `/tutor` | The AI feature. Any topic → explanation + 3 MCQs + next lesson. |
...
| Feature | Detail |
| --- | --- |
| **Live AI tutoring** | Any topic, not a fixed list. Explanation + 3 MCQs + next lesson, generated per request. |
 ...
 /tutor  ────────── Server Component reads ?subject= & ?topic=
      ...
      └────────────────────────────────>│
                         ...
                         shuffleMcqOptions() = answer position
                                        │
                         { ok, model, topic, explanation, mcqs, nextLesson }
                                        │
      <─────────────────────────────────┘
...
| `app/globals.css` | The entire design system: `pk-*` green scale, clay shadows, light/dark tokens, focus ring, reduced motion. |
...
**Prompting.** One system prompt fixes the persona (a tutor for grades 5–10),
the register (simple English plus Roman Urdu, Pakistani examples), and the exact
JSON shape, with `response_format: { type: "json_object" }`, `temperature: 0.4`,
`max_tokens: 1800`, and a 45-second timeout.

**Never trust the model.** Everything a model returns passes through
`parseTutorLesson`, which:

- requires a non-empty top-level `explanation`;
...
- returns `null` on anything unusable, which the client turns into a typed error.
...
Save captures into `docs/screenshots/` using the filenames below and the images
appear here automatically — the paths are already wired up.

| Screen | File | What to capture |
| --- | --- | --- |
| Home | `docs/screenshots/home.png` | Hero with the positioning line plus the five subject cards. |
| AI Tutor | `docs/screenshots/tutor.png` | A generated lesson: explanation, three MCQs, next lesson. |
