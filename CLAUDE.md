...
- `lib/tutor.ts` — the shared contract, imported by both server and client: `SUBJECTS`/`isSubject`, `TutorLesson`, the `TutorApiResponse` union, `MAX_TOPIC_LENGTH`, `MCQ_COUNT`, and `parseTutorLesson`. That parser is the trust boundary — it revalidates every model response (tolerating `correctIndex` sent as `"B"` or `"2"`, enforcing exactly 4 options) so a bad reply becomes a typed error, not a broken render. It also owns two things the browser depends on: `studentErrorMessage(code)`, the Roman-Urdu translation table the route uses so raw upstream text ("Add 10 credits to unlock…") never reaches a student, and `shuffleMcqOptions()`.
- **Always shuffle generated MCQs.** Every free model tested returns `correctIndex: 0` for all three questions, so an unshuffled lesson is answerable with "always pick A". `lib/openrouter.ts` applies `shuffleMcqOptions` after a lesson validates — deliberately outside `parseTutorLesson`, so validation stays deterministic.
- `app/api/tutor/route.ts` — `force-dynamic`. Validates the body, applies a 12-request/60s in-memory limit keyed on `x-forwarded-for` **before** parsing the body, and returns `{ ok: true, model, ...lesson }` or `{ ok: false, code, error }`. `fail()` translates the code through `studentErrorMessage` and attaches the raw upstream text as `detail` **only when `NODE_ENV !== "production"`**; the full message is always `console.error`-logged server-side. The `GET` handler is a dev-only config check (key presence and model chain, never the key itself) and 404s in production. The limiter is per-instance, so serverless needs a shared store for real traffic.

Success payloads are **flat**, not nested under a `lesson` key — clients narrow on `payload.ok` and then read `payload.explanation` / `payload.mcqs` / `payload.nextLesson` directly. On failure they read `payload.error` (already student-facing) and may read `payload.detail`, which is present in development only.
...
### Chrome and the design system

- `lib/site.ts` — `SITE_NAME`, `SITE_TAGLINE` ("AI-Powered Learning for Every Student in Pakistan"), `SITE_TAGLINE_UR`, `GITHUB_URL`, `NAV_LINKS`. Header, footer, and page metadata all read from here, so renaming a route or the tagline is a one-file change.
- `components/site-header.tsx` — the only Client Component in the chrome, because the active nav item needs `usePathname`. One wrapping flex row reordered with `order-*`: on phones the nav is a full-width 4-column grid of 44px targets below the logo, from `md` up it is inline. Active links get `aria-current="page"`; `isActive` special-cases `/` so it does not match every route, and prefix-matches otherwise so `/tutor?subject=…` stays lit.
- `components/site-footer.tsx` — Server Component. Brand block, feature links derived from `NAV_LINKS`, subject deep links into `/tutor?subject=`, privacy line, repo link.
- `app/globals.css` is the whole design system: the `pk-*` green scale anchored on `#01411c`, `--shadow-clay*`, light/dark tokens, the global `:focus-visible` ring, and a `prefers-reduced-motion` block that collapses every duration. Two fonts — Lexend for `h1`–`h4`, Geist for body — with the heading rule inside `@layer base` so `font-sans`/`font-mono` utilities can still win. `html { scroll-padding-top }` is set there too, because the sticky header would otherwise cover anchor targets (including the skip link's `#main`).
